<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-2xl text-gray-900 leading-tight">
            Tambah Log Perawatan untuk {{ $pet->name }}
        </h2>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm rounded-xl">
                <div class="p-8">
                    <form action="{{ route('pets.care-logs.store', $pet) }}" method="POST">
                        @csrf
                        ...
                        <div class="mb-8">
                            ...
                            <textarea name="description" id="description" rows="4" 
                                ...
                                class="w-full border-gray-300 rounded-lg shadow-sm focus:border-gray-900 focus:ring-gray-900">{{ old('description') }}</textarea>
                            @error('description')
                                <p class="text-red-600 text-sm mt-2">{{ $message }}</p>
                            @enderror
                        </div>
